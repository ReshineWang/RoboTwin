...
class MVActor:
    ...
    def play(self, obs, prompt, idx=0, num_inference_steps=5, noise_scale=0.03, execution_step=1, state=None):
        ...
        # 添加 resize 逻辑
        target_h, target_w = self.resize  # 从配置中获取目标大小
        if (h, w) != (target_h, target_w):
            resized_obs = []
            for i in range(v):  # 遍历每个视角
                img = obs[i].permute(1, 2, 0).cpu().numpy()  # 转换为 (h, w, c)
                img = cv2.resize(img, (target_w, target_h), interpolation=cv2.INTER_LINEAR)  # 调整大小
                img = torch.tensor(img).permute(2, 0, 1)  # 转换回 (c, h, w)
                resized_obs.append(img)
            obs = torch.stack(resized_obs, dim=0)  # 合并为 (v, c, h, w)
            h = target_h
            w = target_w
        ...
        if self.add_state:
            # 防御性地调整 state 维度，避免与 action_dim 不匹配导致后续广播报错
            if state is None:
                raise ValueError("state is required when add_state=True")
            if state.shape[-1] != self.action_dim:
                if state.shape[-1] > self.action_dim:
                    state = state[..., :self.action_dim]
                else:
                    pad = self.action_dim - state.shape[-1]
        ...
        # 部分权重会返回比 execution_step 更长的时间维，这里对齐以避免后续维度不一致
        t_pred = actions_pred.shape[1]
        eff_step = min(execution_step, t_pred)
        actions_pred = actions_pred[:, :eff_step, :]
